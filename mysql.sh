...
if [ ! -f "/usr/local/mysql-8.0.13-linux-glibc2.12-x86_64.tar.xz" ];then
echo "正在下载mysql..."
else
rm -rf /usr/local/mysql-8.0.13-linux-glibc2.12-x86_64.tar.xz
echo "正在下载mysql..."
fi
wget https://cdn.mysql.com//Downloads/MySQL-8.0/mysql-8.0.13-linux-glibc2.12-x86_64.tar.xz
if [ -d "/usr/local/mysql-8.0.13-linux-glibc2.12-x86_64" ];then
echo "/usr/local/mysql-8.0.13-linux-glibc2.12-x86_64文件夹存在"
rm -rf mysql-8.0.13-linux-glibc2.12-x86_64
echo "文件解压中..."
else
echo "文件解压中..."
fi
